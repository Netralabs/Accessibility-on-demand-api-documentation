"""
1_upload_from_url.py (async)  —  STEP 1 (option B): Upload from signed URLs
==========================================================================
...
async def main():
    ...
    if response.status_code == 413:
        ...
        print("                    the 1 GB per-request limit, so nothing was uploaded.")
        print("                    Send fewer URLs per request (there's no limit on the number")
        print("                    of URLs, only their combined size), then try again.")
        try:
            raw = response.json()
        except ValueError:
            raw = None
