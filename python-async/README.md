...
All commands are run from inside `python-async/`. The three files you care about:

| Purpose | File | Path (from inside `python-async/`) |
|---------|------|--------------------------------|
...
- To **see results**, open `python-async/data.json` after running a step.
- To **see failures**, open `python-async/errors.json` (created only when something goes wrong).
...
> ⏳ The download link expires (see `expires_in_seconds`, e.g. 604800 = 7 days; longer values are possible). Download the PDF soon.
